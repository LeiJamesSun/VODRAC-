function [pts_3d,pts_3d_,R,t]=Build_Scenario(n_ele,noise,outlier_ratio,scale_gt,show_figure)

%% real data (bunny)
% load source point cloud
pcd = pcread('data/bunny.ply'); %data/bunny T-rex_high_0
...
pcd_up= pcd; %0.029 for bunny 0.0065 for Ar 0.0001 for Lucy

pts_3dsp = 6*pcd_up.Location; % source points %6 for bunny 0.0065 for Ar 0.00063 for Lucy

pts_3dsp=double(pts_3dsp(1:(pcd_up.Count),:));

pts_3dsp=pts_3dsp-0.5*[max(pts_3dsp(:,1))+min(pts_3dsp(:,1)), max(pts_3dsp(:,2))+min(pts_3dsp(:,2)), max(pts_3dsp(:,3))+min(pts_3dsp(:,3))];
...
pts_3dsp_=scale_gt * pts_3dsp * R'+t+noise*randn(pcd_up.Count,3);
...
for i=1:round(n_ele*outlier_ratio)
    ...
    for iii=1:1e+12
    rand_vec=2*scale_gt*rand(1,3)-1*scale_gt;
...
% pts_3d_(i,:)=pts_3dsp_(1+round(1+(pcd_up.Count-2)*rand(1)),:); %[0.0,0.0,0.0]+t+rand_vec;
